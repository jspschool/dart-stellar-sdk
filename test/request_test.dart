...
void testEffectsRequestBuilder() {
  Server server = new Server("https://horizon-testnet.stellar.org");
  Uri uri =
      server.effects.limit(200).order(RequestBuilderOrder.DESC).buildUri();
  assert("https://horizon-testnet.stellar.org/effects?limit=200&order=desc" ==
      uri.toString());
}
...
void testEffectsRequestBuilderForAccount() {
  ...
  Uri uri = server.effects
      .forAccount(KeyPair.fromAccountId(
...
void testEffectsRequestBuilderForTransaction() {
  ...
  Uri uri = server.effects
      .forTransaction(
          "991534d902063b7715cd74207bef4e7bd7aa2f108f62d3eba837ce6023b2d4f3")
      .buildUri();
  assert(
...
void testEffectsRequestBuilderForOperation() {
  Server server = new Server("https://horizon-testnet.stellar.org");
  Uri uri =
      server.effects.forOperation(28798257847).cursor("85794837").buildUri();
  assert(
...
void testLedgersRequestBuilder() {
  Server server = new Server("https://horizon-testnet.stellar.org");
  Uri uri = server.ledgers.limit(200).order(RequestBuilderOrder.ASC).buildUri();
  assert("https://horizon-testnet.stellar.org/ledgers?limit=200&order=asc" ==
      uri.toString());
}
...
void testOperationsRequestBuilder() {
  Server server = new Server("https://horizon-testnet.stellar.org");
  Uri uri =
      server.operations.limit(200).order(RequestBuilderOrder.DESC).buildUri();
  assert(
      "https://horizon-testnet.stellar.org/operations?limit=200&order=desc" ==
          uri.toString());
}
...
void testOrderBookRequestBuilder() {
  ...
  Uri uri = server.orderBook
      .buyingAsset(Asset.createNonNativeAsset(
          "EUR",
  ...
  assert("https://horizon-testnet.stellar.org/order_book?" +
...
void testPathsRequestBuilder() {
  ...
  Uri uri = server.paths
      .destinationAccount(KeyPair.fromAccountId(
          "GB24QI3BJNKBY4YNJZ2I37HFIYK56BL2OURFML76X46RQQKDLVT7WKJF"))
      .sourceAccount(KeyPair.fromAccountId(
          "GD4KO3IOYYWIYVI236Y35K2DU6VNYRH3BPNFJSH57J5BLLCQHBIOK3IN"))
      .destinationAmount("20.50")
      .destinationAsset(Asset.createNonNativeAsset(
          "USD",
          KeyPair.fromAccountId(
...
void testPaymentsRequestBuilder() {
  Server server = new Server("https://horizon-testnet.stellar.org");
  Uri uri =
      server.payments.limit(200).order(RequestBuilderOrder.DESC).buildUri();
  assert("https://horizon-testnet.stellar.org/payments?limit=200&order=desc" ==
      uri.toString());
}
...
void testTradesRequestBuilder() {
  ...
  Uri uri = server.trades
      .baseAsset(Asset.createNonNativeAsset(
          "EUR",
          KeyPair.fromAccountId(
              "GAUPA4HERNBDPVO4IUA3MJXBCRRK5W54EVXTDK6IIUTGDQRB6D5W242W")))
      .counterAsset(Asset.createNonNativeAsset(
          "USD",
          KeyPair.fromAccountId(
              "GDRRHSJMHXDTQBT4JTCILNGF5AS54FEMTXL7KOLMF6TFTHRK6SSUSUZZ")))
      .cursor("13537736921089")
...
void testTransactionsRequestBuilder() {
  Server server = new Server("https://horizon-testnet.stellar.org");
  Uri uri =
      server.transactions.limit(200).order(RequestBuilderOrder.DESC).buildUri();
  assert(
